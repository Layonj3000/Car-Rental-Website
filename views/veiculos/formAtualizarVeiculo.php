...
<form action="../../controlers/controlerVeiculo.php">
    ...
    <div class="padrao">
        ...
        <input type="text" name="nomeVeiculo" value=<?= $veiculo -> getNome()?> required>
    </div>
    
    <div class="padrao">
        <label for="anoFabricacao">Ano de Fabricacao:</label>
        <input type="number" min="1900" max="2100" name="anoFabricacao" value=<?= $veiculo -> getAnoFabricacao()?> required>
    </div>
    
    <div class="padrao">
        <label for="fabricante">Fabricante:</label>
        <input type="text" name="fabricante" value=<?= $veiculo -> getFabricante()?> required>
    </div>

    <div class="padrao">
        <label for="motorizacao">Motorização:</label>
        <input type="text" name="motorizacao" value=<?= $veiculo -> getMotorizacao()?> required>
    </div>
    
    <div class="padrao">
        <label for="valorBase">Valor Base:</label>
        <input type="text" name="valorBase" value=<?= $veiculo -> getValorBase()?> required>
    </div>
    
    <div class="padrao">
        <label for="opcionais">Opcionais:</label>
        <input type="text" name="opcionais" value=<?= $veiculo -> getOpcionais()?> required>
    </div>
